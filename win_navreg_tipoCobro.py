...
querysql_update = "update tipocobro set idtipoCobro = {}, Descrip = '{}' where idtipoCobro = {}"
querysql_insert = "insert into tipocobro (idtipocobro, Descrip) values ({}, '{}')"
querysql_delete = "delete from tipocobro where idtipocobro = {}"
querysql_select = "select idtipocobro, descrip from tipocobro order by idtipocobro"
querysql_selectmax = "select ifnull(max(idtipocobro), 0) + 1 from tipocobro"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba
etiquetascol = ["Código", "Descripcion"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        querysql_permiso = "select valor from usuariopermisos where idusuarios like "+idusuario+" and idpermisos like 4"
        cursor.execute(querysql_permiso)
        self.permiso = cursor.fetchone()

        self.vnav = uic.loadUi("win_navreg_tipoCobro.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_tipoCobro.ui")
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            ...
            filename = "TipoCobro {}.pdf".format(str(vvfecha)) #Nombre del archivo

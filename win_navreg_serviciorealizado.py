...
querysql_delete = "delete from servicio where idservicio = {}"
querysql_delete2 = "delete from detservicio where idservicio = {}"
...
etiquetascol = ["idServicio", "Nro Servicio", "idCliente", "Razón Social", "RUC-CI", "Fecha", "idUsuario", "Usuario"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_serviciorealizado.ui")
    ...
    def activated_tableWidget(self): #Se activa al darle Enter con una fila seleccionada
        if self.mylistaapegar is None:
            ...
            try:
                ...
                self.mylistaapegar[1].setText(v2)
                self.mylistaapegar[2].setText(v3) #     
                self.vnav.close()
            except:
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_serviciorealizado.ui")
            ...
            self.vedit.lineEdit_6.setText(v11)
            ...
            self.vedit.pushButton_8.clicked.connect(self.clicked_pushButton_8)
    ...
    def clicked_pushButton_4(self): #Busca items para el det
        ...
        self.listalineedit_pks_items = [self.vedit.lineEdit_12, self.vedit.lineEdit_22, self.vedit.lineEdit_13, self.vedit.lineEdit_4, self.vedit.lineEdit_20]
        self.run_win_navreg_items = win_navreg_items.iniciar(self.conex, self.listalineedit_pks_items)

    def clicked_pushButton_8(self): #Busca SERVICIOS para el det
        import win_navreg_items
        self.listalineedit_pks_items = [self.vedit.lineEdit_12, self.vedit.lineEdit_22, self.vedit.lineEdit_13, self.vedit.lineEdit_4, self.vedit.lineEdit_20]
        self.run_win_navreg_items = win_navreg_items.iniciar(self.conex, self.listalineedit_pks_items, 1)

    

    def click_actionaceptar(self):
        v0 = self.vedit.lineEdit.text() #
        v1 = self.vedit.lineEdit_2.text() #Lee los datos del LineEdit -- #idCliente
        v2 = self.vedit.lineEdit_9.text()
        v3 = self.vedit.lineEdit_3.text()
        vfecha = self.vedit.dateEdit.dateTime().toString("yyyy-MM-dd") #fecha
        
        v11 = self.vedit.lineEdit_6.text() #NroServicio
       ...
       # v25 = self.vedit.lineEdit_6.text() #TipoCobro
        ...
        if len(v0) > 0 and len(v1) > 0 and len(v23) > 0:
            if v0.isnumeric():
                    ...
                    celda5 = QtWidgets.QTableWidgetItem(v11)
                    ...
                    try:
                        ...
                        if self.cantfilas > 0:
                            ...
                            filename = "Orden de Servicio {}.pdf".format(str(vvfecha)) #Nombre del archivo
                            ...
                            html += "<h1>{}</h1>".format("Servicio Realizado")
                            html += """
                                    <body>
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            now = datetime.now()
            vvfecha = now.strftime("%d-%m-%Y, %H-%M-%S")
            filename = "Orden de Venta {}.pdf".format(str(vvfecha)) #Nombre del archivo

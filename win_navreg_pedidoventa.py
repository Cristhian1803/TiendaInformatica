...
querysql_delete = "delete from pedidoventa where idpedidoventa = {}"
querysql_delete2 = "delete from detpedidoventa where idpedidoventa = {}"
...
select
    ...
    p.razonsocial,
    p.ruc_ci,
    pr.fecha from Pedidoventa pr
        inner join clientes p on pr.idClientes = p.idClientes
    order by pr.idPedidoventa
"""
querysql_selectmax = "select ifnull(max(idpedidoventa), 0) + 1 from pedidoventa"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba



etiquetascol = ["ID Pedido de Venta", "id Proveedor", "Razón Social", "RUC-CI", "Fecha"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_pedidoventa.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_pedidoventa.ui")

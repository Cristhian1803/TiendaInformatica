...
querysql_insert = "insert into cuentaspagar (nroCuenta, NroFactura, fecha, nroCuotas, importe) values ({}, {}, '{}', {}, {})"
querysql_insert2 = "insert into pagocuota (idpagocuota, NroCuenta, nroCuota, fechacaducidad) values ({}, {}, {}, '{}')"
querysql_delete = "delete from cuentaspagar where nroCuenta = {}"
querysql_delete2 = "delete from pagocuota where nroCuenta = {}"
querysql_select = "select cc.nroCuenta, cc.NroFactura, fc.idproveedores, p.razonsocial, p.ruc_ci, cc.fecha, cc.nroCuotas, cc.Importe, cc.importe / cc.nrocuotas as mensualidad from cuentaspagar cc inner join compras_encab fc on cc.NroFactura = fc.NroFactura inner join proveedores p on fc.idProveedores = p.IdProveedores order by nroCuenta"
querysql_selectmax = "select ifnull(max(nroCuenta), 0) + 1 from cuentaspagar"
...
etiquetascol = ["Nro de Cuenta", "Nro Factura", "idProveedor", "Razon Social", "RUC-CI", "Fecha Emision", "Nro de Cuotas", "Importe Total", "Mensualidad"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_cuentaspagar.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_cuentaspagar.ui")
                self.vedit.label_3.setText(etiquetascol[0] + ": ")
                self.vedit.label_6.setText("Nro Factura: ")
                self.vedit.label_4.setText("Fecha Emision: ")
                self.vedit.label.setText("Nro de Cuotas: ")
                self.vedit.label_5.setText("Monto Total: ")
    ...
    def clicked_pushButton(self):
        ...
        self.listalineedit_pks_compra = [self.vedit.lineEdit_3, self.vedit.lineEdit_2, self.vedit.lineEdit_6, self.vedit.lineEdit_7]
    ...
    def click_actionaceptar(self):
        v0 = self.vedit.lineEdit.text() #Lee los datos del LineEdit -- NroCuenta
        v1 = self.vedit.lineEdit_3.text() #NroFactura
        v2 = self.vedit.lineEdit_2.text()#idProveedores
        vfecha = self.vedit.dateTimeEdit.dateTime().toString("yyyy-MM-dd HH:mm:ss")
        v3 = self.vedit.lineEdit_6.text()
        v4 = self.vedit.lineEdit_7.text()
        v5 = self.vedit.lineEdit_4.text() #Cuotas
        v6 = self.vedit.lineEdit_5.text() #Monto


        vfila = self.filaamodificar
        if len(v0) > 0 and len(v1) > 0 and len(v5) > 0 and len(v6) > 0:
            if v6.isnumeric() and v5.isnumeric():
                ...
                if self.editando: #Modificando
                    ...
                    cursor.execute(querysql_insert.format(v0, v1, vfecha, v5, v6)) #SQL ejecutandose NUEVO 
                    ...
                    #Detalle de Cuotas
                    nroCuota = 1
                    cursor.execute(querysql_selectmax2)
                    res2 = cursor.fetchone()
                    IDPago = res2[0]
                    vvfecha = self.vedit.dateTimeEdit.dateTime()
                    fechaAgregar = vvfecha.toPyDateTime()
                    v5 = int(v5)
                    for index in range (v5): 
                        fechaAgregar = fechaAgregar + timedelta(days=30)
                        fechaguardar = fechaAgregar.strftime("%Y-%m-%d")
                        cursor.execute(querysql_insert2.format(IDPago, v0, nroCuota, fechaguardar))
                        nroCuota = nroCuota + 1
                        cursor.execute(querysql_selectmax2)
                        res2 = cursor.fetchone()
                        IDPago = res2[0]
                        print("index cuentaPagar ",index)
                ...
                self.vnav.tableWidget.setItem(vfila, 5, celdafech)
    ...
    def cargardatos(self):
        ...
        for campoid, campodescrip, campo3, campo4, campo5,campo6, campo7, campo8, campo9 in cursor: #

...
querysql_insert = "insert into cobros (idCobro, NroCuenta, monto, descrip, fecha, idapertura, idusuarios) values ({}, {}, {}, '{}', '{}', {}, {})"
querysql_insert2 = "insert into pagocuota (idpagocuota, NroCuenta, nroCuota, fechacaducidad) values ({}, {}, {}, '{}')"
querysql_delete = "delete from cobros where idcobro = {}"
...
select
    ...
    us.usuario
    from cobros cc
    inner join cuentascobrar cpa on cc.NroCuenta = cpa.NroCuenta 
    inner join ventas_encab fc on cpa.NroFactura = fc.NroFactura 
    inner join clientes p on fc.idClientes = p.IdClientes
    inner join aperturacaja ac on cc.idapertura = ac.idapertura
    inner join usuarios us on cc.IdUsuarios = us.IdUsuarios
"""
querysql_selectmax = "select ifnull(max(idcobro), 0) + 1 from cobros"
...
etiquetascol = ["idCobro", "NroCuenta", "Nro Factura", "idProveedor", "Razon Social", "RUC-CI", "Monto", "Descripcion", "Fecha", "IdApertura", "idUsuario", "Usuario"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_cobros.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_cobros.ui")
                self.vedit.label_3.setText(etiquetascol[0] + ": ")
                self.vedit.label_6.setText("Nro Cuenta: ")
    ...
    def clicked_pushButton(self):
        ...
        self.listalineedit_pks_cuentascobrar = [self.vedit.lineEdit_2, self.vedit.lineEdit_9, self.vedit.lineEdit_10, self.vedit.lineEdit_6, self.vedit.lineEdit_7]
        self.run_win_navreg_cuentascobrar = win_navreg_cuentascobrar.iniciar(self.conex, self.listalineedit_pks_cuentascobrar)
    ...
    def click_actionaceptar(self):
        ...
        if len(v0) > 0 and len(v2) > 0 and len(v3) > 0 and len(v4) > 0 and len(v5) > 0:
            if v2.isnumeric():
                ...
                if self.editando: #Modificando
                    ...
                    cursor.execute(querysql_insert.format(v0, v11, v2, v3, vfecha, v4, v5)) #SQL ejecutandose NUEVO 
                ...
                self.vnav.tableWidget.setItem(vfila, 1, celda2)
                ...
                self.vnav.tableWidget.setItem(vfila, 8, celdafech)
                self.vnav.tableWidget.setItem(vfila, 9, celda9)
                self.vnav.tableWidget.setItem(vfila, 10, celda10)
                self.vnav.tableWidget.setItem(vfila, 11, celda11)#
    ...
    def click_actionEliminar(self):
        try: #Pregunta
            ...
            txt_valor1 = self.vnav.tableWidget.selectedIndexes()[4].data()
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            ...
            filename = "Cobros {}.pdf".format(str(vvfecha)) #Nombre del archivo
    ...
    def cargardatos(self):
        ...
        for campoid, campodescrip, campo4, campo5,campo6, campo7, campo8, campo9, \
            ...
            self.vnav.tableWidget.setItem(fila, 2, cel3)
            self.vnav.tableWidget.setItem(fila, 3, cel4) 
            self.vnav.tableWidget.setItem(fila, 4, cel5)
            self.vnav.tableWidget.setItem(fila, 5, cel6)
            self.vnav.tableWidget.setItem(fila, 6, cel7)
            self.vnav.tableWidget.setItem(fila, 7, cel8)
            self.vnav.tableWidget.setItem(fila, 8, cel9)
            self.vnav.tableWidget.setItem(fila, 9, cel10)
            self.vnav.tableWidget.setItem(fila, 10, cel11)
            self.vnav.tableWidget.setItem(fila, 11, cel12)#
            fila += 1

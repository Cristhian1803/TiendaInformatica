...
querysql_update = "update proveedores set idProveedores = {}, RazonSocial = '{}', RUC_CI = '{}', Direccion = nullif(trim('{}'),''), TelefCel = nullif(trim('{}'),'') where idProveedores = {}"
querysql_insert = "insert into proveedores (idProveedores, RazonSocial, RUC_CI, Direccion, TelefCel) values ({}, '{}', '{}', nullif(trim('{}'),''), nullif(trim('{}'),''))"
querysql_delete = "delete from proveedores where idProveedores = {}"
querysql_select = "select idProveedores, RazonSocial, RUC_CI, Direccion, TelefCel from proveedores order by idProveedores"
querysql_selectmax = "select ifnull(max(idProveedores), 0) + 1 from proveedores"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba
etiquetascol = ["Código", "Razón Social", "RUC CI", "Dirección", "Teléfono"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        querysql_permiso = "select valor from usuariopermisos where idusuarios like "+idusuario+" and idpermisos like 2"
        cursor.execute(querysql_permiso)
        self.permiso = cursor.fetchone()

        self.vnav = uic.loadUi("win_navreg_proveedores.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_proveedores.ui")
    ...
    #def clicked_pushButton_ciudad(self):
     #   import win_navreg_ciudad
      #  self.listalineedit_pks_ciudad = [self.vedit.lineEdit_6, self.vedit.lineEdit_6_2]
       # self.run_win_navreg_ciudad = win_navreg_ciudad.iniciar(self.conex, self.listalineedit_pks_ciudad)
    ...
    def click_actionaceptar(self):
        ...
        if len(v0) > 0 and len(v1) > 0 and len(v2) > 0 and len(v4) > 0:
            ...
            try:
                ...
                if self.editando: #Modificando
                    ...
                    cursor.execute(querysql_insert.format(v0, v1, v2, v3, v4)) #SQL ejecutandose NUEVO 
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            ...
            filename = "proveedores {}.pdf".format(str(vvfecha)) #Nombre del archivo

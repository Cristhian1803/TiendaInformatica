...
querysql_delete = """
 update ventas_encab set 
    idFactura = {},
    idTimbrado = nullif(trim({}),''),
    NroFactura = nullif(trim({}),''),
    FechaFactura = nullif(trim('{}'),''),
    Condicion = nullif(trim('{}'),''),
    Anulado = {},
    idClientes = {},
    idTiposFacturas = {},
    idUsuarios = nullif(trim({}),'')
      where idFactura = {}
"""
querysql_select = """
select
    ...
    order by fv.idservicio
"""
querysql_selectmax = "select ifnull(max(idservicio), 0) + 1 from servicio"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba



etiquetascol = ["Código", "Nro Servicio", "Código Timbrado", "Timbrado", "idCliente", "Razón Social", "RUC/CI", "Fecha de Emisión", "idUsuario", "Usuario"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_ventas.ui")
    ...
    def activated_tableWidget(self): #Se activa al darle Enter con una fila seleccionada
        if self.mylistaapegar is None:
            ...
            try:
                ...
                v1 = self.vnav.tableWidget.selectedIndexes()[5].data()
                v2 = self.vnav.tableWidget.selectedIndexes()[6].data() #
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_ventas.ui")
                self.vedit.label.setText(etiquetascol[0] + ": ")
                self.vedit.label_3.setText("Timbrado: ")
                self.vedit.label_4.setText(etiquetascol[3] + ": ")
                self.vedit.label_5.setText(etiquetascol[4] + ": ")
                self.vedit.label_6.setText("Cliente: ")
                #self.vedit.label_7.setText("Tipo de Cobro: ")
            ...
            #self.vedit.lineEdit_8.setText(v12)
            ...
            self.vedit.pushButton_8.clicked.connect(self.deleteDetalle)
    ...
    def deleteDetalle (self):
        txt_filaAEliminarDet = self.vedit.tableWidget2.currentRow()
        self.vedit.tableWidget2.removeRow(txt_filaAEliminarDet)

    def clicked_pushButton(self):
        import win_navreg_timbrado
        self.listalineedit_pks_tim = [self.vedit.lineEdit_2, self.vedit.lineEdit_9, self.vedit.lineEdit_23, self.vedit.lineEdit_24]
        self.run_win_navreg_timbrado = win_navreg_timbrado.iniciar(self.conex, self.listalineedit_pks_tim)

    def clicked_pushButton_2(self):
        import win_navreg_clientes
        self.listalineedit_pks_cli = [self.vedit.lineEdit_4, self.vedit.lineEdit_5, self.vedit.lineEdit_6]
        self.run_win_navreg_clientes = win_navreg_clientes.iniciar(self.conex, self.listalineedit_pks_cli)
    ...
    def click_actionaceptar(self):
        ...
        v7 = self.vedit.lineEdit_4.text() #idCliente
        v9 = self.vedit.lineEdit_5.text()
        v10 = self.vedit.lineEdit_6.text()
        #v11 = self.vedit.lineEdit_7.text() #idTipoCobro
        #v12 = self.vedit.lineEdit_8.text()
        ...
        if len(v0) > 0 and len(v1) > 0 and len(v3) > 0 and len(v7) > 0 and len(v13) > 0 and len(v23) > 0:
            if v0.isnumeric():
                    ...
                    try:
                        ...
                        if self.editando: #Modificando
                            ...
                            cursor.execute(querysql_insert.format(v0, v1, v3, vfecha, v7, v13)) #SQL ejecutandose NUEVO 
                            ...
                            for row in range (nb_row):
                                ...
                                colum4 = self.vedit.tableWidget2.item(row, 5).text()
                                cursor.execute(querysql_insert2.format(v0, colum1, colum2, colum3, colum4))
                        ...
                        self.vnav.tableWidget.setItem(vfila, 6, celda9)
                        self.vnav.tableWidget.setItem(vfila, 7, celda10)
                        self.vnav.tableWidget.setItem(vfila, 8, celda13)
                        self.vnav.tableWidget.setItem(vfila, 9, celda14) #
                        ...
                        if self.cantfilas > 0:
                            ...
                            filename = "FacturaVenta {}.pdf".format(str(vvfecha)) #Nombre del archivo
                                            ...
                                            <table>
                                                <tr>
                                                    ...
                                                    <td>
                                                        Timbrado: {} <br />
                                                        RUC: 4029146-3 <br />
                                                        Vigencia del Timbrado: {} <br />
                                                        Vencimiento del Timbrado: {} <br />
                                                        Factura #{}<br />
                                                        Fecha de Emisión: {}
                                                    </td>
                                                </tr>
                        ...
                        <br><br>
                                    """.format(v2, v24, v25, v3, vfecha, v7, v9, v10, v14)
    ...
    def click_actionEliminar(self):
        try: #Pregunta
            ...
            v1 = self.vnav.tableWidget.selectedIndexes()[1].data()
            v3 = self.vnav.tableWidget.selectedIndexes()[3].data()
            vfecha = self.vnav.tableWidget.selectedIndexes()[4].data()
            v4 = self.vnav.tableWidget.selectedIndexes()[5].data()
            vchk6 = 1
            v7 = self.vnav.tableWidget.selectedIndexes()[7].data()
            v11 = self.vnav.tableWidget.selectedIndexes()[10].data()
            v13 = self.vnav.tableWidget.selectedIndexes()[12].data()
            txt_valor0 = self.vnav.tableWidget.selectedIndexes()[0].data()
            txt_valor1 = self.vnav.tableWidget.selectedIndexes()[8].data()
            txtmensaje = "<span style='font-size: 14pt; color: #cd1014;'><b>Estas seguro que desea anular?</b></span><span style='font-size: 12pt; color: #333333;'><p>Codigo: <b>{}</b><br>Nombre: <b>{}</b></span>".format(txt_valor0, txt_valor1)
            resp_usu = preguntar(txtmensaje)
            self.filaamodificar = self.vnav.tableWidget.currentRow()
            vvvfila = self.filaamodificar
            if resp_usu: #Proceso de eliminacion 
                celda000 = QtWidgets.QTableWidgetItem(v0)
                celda001 = QtWidgets.QTableWidgetItem(v1)
                celda002 = QtWidgets.QTableWidgetItem(v3)
                celda003 = QtWidgets.QTableWidgetItem(vfecha)
                celda004 = QtWidgets.QTableWidgetItem(v4)
                celda005 = QtWidgets.QTableWidgetItem()
                if vchk6 ==0:
                    celda005.setCheckState(QtCore.Qt.Unchecked)
                else:
                    celda005.setCheckState(QtCore.Qt.Checked)
                celda006 = QtWidgets.QTableWidgetItem(v7)
                celda007 = QtWidgets.QTableWidgetItem(v11)
                celda008 = QtWidgets.QTableWidgetItem(v13)
                try: 
                    cursor = self.conex.cursor()
                    cursor.execute(querysql_update.format(v0, v1, v3, vfecha, v4, vchk6, v7, v11, v13, v0))
                    self.conex.commit()

                    celda000.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsSelectable)
                    celda001.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsSelectable)
                    celda002.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsSelectable)
                    celda003.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsSelectable)
                    celda004.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsSelectable)
                    celda005.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsSelectable)
                    celda006.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsSelectable)
                    celda007.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsSelectable)
                    celda008.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsSelectable)

                    self.vnav.tableWidget.setItem(vvvfila, 0, celda000)
                    self.vnav.tableWidget.setItem(vvvfila, 1, celda001)
                    self.vnav.tableWidget.setItem(vvvfila, 3, celda002)
                    self.vnav.tableWidget.setItem(vvvfila, 4, celda003)
                    self.vnav.tableWidget.setItem(vvvfila, 5, celda004)
                    self.vnav.tableWidget.setItem(vvvfila, 6, celda005)
                    self.vnav.tableWidget.setItem(vvvfila, 7, celda006)
                    self.vnav.tableWidget.setItem(vvvfila, 10, celda007)
                    self.vnav.tableWidget.setItem(vvvfila, 12, celda008)

                except:
                   mensajebox("<html><head/><body><p><span style=' font-size:12pt; font-weight:600; color:#7a2e2e;'>No se puede anular este registro.</span></p><p><span style=' font-size:10pt; font-weight:600; color:#414141;'>Es posible que se esté utilizando como referencial en otra tabla.</span></p></body></html>")
        except:
           self.vnav.statusbar.showMessage("Seleccione un registro para anular")
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            now = datetime.now()
            vvfecha = now.strftime("%d-%m-%Y, %H-%M-%S")
            filename = "ventas {}.pdf".format(str(vvfecha)) #Nombre del archivo
    ...
    def cargardatos(self):
        ...
        for campoid, campodescrip, campo3, campo4, campo5, \
            campo8, campo9, campo10, campo11, campo12 in cursor: #
            ...
            cel7 = QtWidgets.QTableWidgetItem(campo8)

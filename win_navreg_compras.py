...
select
    ...
    fc.FechaFactura,
    fc.idUsuarios, 
    us.usuario from compras_encab fc
        inner join proveedores p on fc.idProveedores = p.idProveedores
        inner join usuarios us on fc.IdUsuarios = us.IdUsuarios
    order by fc.NroFactura
"""
querysql_selectmax = "select ifnull(max(NroFactura), 0) + 1 from compras_encab"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba



etiquetascol = ["Nro Factura", "idFactura", "idProveedor", "Razón Social", "RUC/CI", "Fecha de Emisión", "idUsuario", "Usuario"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_compras.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                ...
                self.vedit.label_16.setText("IVA 10%: ")
    ...
    def clicked_pushButton_2(self): #Busca tipoCobro
        import win_navreg_tipoCobro
        self.listalineedit_pks_tipco = [self.vedit.lineEdit_8, self.vedit.lineEdit_6]
    ...
    def click_actionaceptar(self):
        ...
        if len(v0) > 0 and len(v1) > 0 and len(v11) > 0 and len(v13) > 0 and len(v23) > 0:
            if v0.isnumeric():
                    ...
                    celda11 = QtWidgets.QTableWidgetItem(v11)
                    ...
                    try:
                        ...
                        if self.editando: #Modificando
                            ...
                            cursor.execute(querysql_insert.format(v0, v11, v1, vfecha, v13)) #SQL ejecutandose NUEVO 
                        ...
                        self.vnav.tableWidget.setItem(vfila, 5, celda11)
                        ...
                        if self.cantfilas > 0:
                            ...
                            filename = "FacturaCompra {}.pdf".format(str(vvfecha)) #Nombre del archivo
                        ...
                        <br><br>
                                    """.format(v0, vfecha, v1, v2, v3, v25, v14)
    ...
    def click_actionEliminar(self):
        try: #Pregunta
            ...
            vchk6 = 1
            v7 = self.vnav.tableWidget.selectedIndexes()[6].data()
            v11 = self.vnav.tableWidget.selectedIndexes()[8].data()
            ...
            if resp_usu: #Proceso de eliminacion 
                ...
                try: 
                    cursor = self.conex.cursor()
                    cursor.execute(querysql_update.format(v0, v1, vfecha, vchk6, v7, v11, v0))
                    self.conex.commit()
                    ...
                    self.vnav.tableWidget.setItem(vvvfila, 5, celda005)
                    self.vnav.tableWidget.setItem(vvvfila, 6, celda006)
                    self.vnav.tableWidget.setItem(vvvfila, 8, celda007)
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            now = datetime.now()
            vvfecha = now.strftime("%d-%m-%Y, %H-%M-%S")
            filename = "compras {}.pdf".format(str(vvfecha)) #Nombre del archivo
    ...
    def click_actionInforme(self):
        import win_filtro_compras
        self.run_win_filtro_compras = win_filtro_compras.iniciar(self.conex)
    ...
    def cargardatos(self):
        ...
        for campoid, campodescrip, campo3, campo4, campo5, \
            campo8, campo9, campo11 in cursor: #
            ...
            self.vnav.tableWidget.setItem(fila, 7, cel10)#
            fila += 1

...
querysql_update = "update aperturacaja set idcierrecaja = {}, montoApertura = {}, RUC_CI = '{}', Direccion = nullif(trim('{}'),''), TelefCel = nullif(trim('{}'),''), ClienteInterno = {} where idclientes = {}"
querysql_insert = "insert into cierrecaja (idcierrecaja, idapertura, montoCierre, fechaCierre, idUsuarios) values ({}, {}, {}, '{}', {})"
querysql_insert2 = "update aperturacaja set abierto = 0 where idapertura = {}"
querysql_delete = "delete from cierrecaja where idcierrecaja = {}"
querysql_select = "select ac.idcierrecaja, c.idapertura, montoCierre, fechaCierre, ac.idUsuarios, us.Usuario from cierrecaja ac inner join aperturacaja c on ac.Idapertura = c.Idapertura inner join usuarios us on ac.IdUsuarios = us.IdUsuarios order by idapertura"
querysql_selectmax = "select ifnull(max(idcierrecaja), 0) + 1 from cierrecaja"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba
etiquetascol = ["Codigo Cierre", "Codigo Apertura", "Monto", "Fecha", "idUsuario", "Usuario"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_cierrecaja.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_cierrecaja.ui")
                self.vedit.label_3.setText(etiquetascol[0] + ": ")
                self.vedit.label_4.setText(etiquetascol[2] + ": ")
                self.vedit.label.setText(etiquetascol[3] + ": ")
                self.vedit.label_5.setText("Usuario: ")
                self.vedit.label_6.setText("Apertura Caja: ")
                ...
                self.vedit.lineEdit_6.setVisible(False)
                self.vedit.lineEdit_7.setVisible(False)
    ...
    def clicked_pushButton(self):
        ...
        self.listalineedit_pks_tim = [self.vedit.lineEdit_3, self.vedit.lineEdit_6, self.vedit.lineEdit_7]
    ...
    def click_actionaceptar(self):
        ...
        v1 = self.vedit.lineEdit_2.text() 
        v2 = self.vedit.lineEdit_3.text()#Lee los datos del LineEdit
        ...
        if len(v0) > 0 and len(v1) > 0 and len(v2) > 0 and len(v3) > 0:
            if v1.isnumeric():
                celda0 = QtWidgets.QTableWidgetItem(v0) #Inserta las filas y columnas despues de validar
                celda1 = QtWidgets.QTableWidgetItem(v1)
                celda5 = QtWidgets.QTableWidgetItem(v2)
                celda2 = QtWidgets.QTableWidgetItem(vfecha)
                ...
                try:
                    ...
                    if self.editando: #Modificando
                        ...
                        cursor.execute(querysql_insert.format(v0, v2, v1, vfecha, v3)) #SQL ejecutandose NUEVO
                        cursor.execute(querysql_insert2.format(v2))
                    ...
                    self.vnav.tableWidget.setItem(vfila, 1, celda5)
                    self.vnav.tableWidget.setItem(vfila, 2, celda1)
                    self.vnav.tableWidget.setItem(vfila, 3, celda2)
                    self.vnav.tableWidget.setItem(vfila, 4, celda3)
                    self.vnav.tableWidget.setItem(vfila, 5, celda4) #
    ...
    def click_actionEliminar(self):
        try: #Pregunta
            ...
            txtmensaje = "<span style='font-size: 14pt; color: #cd1014;'><b>Estas seguro que desea eliminar?</b></span><span style='font-size: 12pt; color: #333333;'><p>Codigo: <b>{}</b><br>Codigo Apertura: <b>{}</b></span>".format(txt_valor0, txt_valor1)
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            ...
            filename = "Cierre Caja {}.pdf".format(str(vvfecha)) #Nombre del archivo
    ...
    def cargardatos(self):
        ...
        for campoid, campodescrip, campo3, campo4, campo5,campo6 in cursor: #

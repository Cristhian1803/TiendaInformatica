...
select
    ...
    p.razonsocial,
    p.ruc_ci,
    pr.fecha from presupuesto pr
        inner join clientes p on pr.idClientes = p.idClientes
    order by pr.idPresupuesto
"""
querysql_selectmax = "select ifnull(max(idPresupuesto), 0) + 1 from presupuesto"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba



etiquetascol = ["idPresupuesto", "idCliente", "Razón Social", "RUC-CI", "Fecha"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_ordenventa.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_ordenventa.ui")
                self.vedit.label.setText(etiquetascol[0] + ": ")
                self.vedit.label_3.setText("Cliente: ")
    ...
    def clicked_pushButton(self): #Busca Clientes
        import win_navreg_clientes
        self.listalineedit_pks_cli = [self.vedit.lineEdit_2, self.vedit.lineEdit_9, self.vedit.lineEdit_3]
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            now = datetime.now()
            vvfecha = now.strftime("%d-%m-%Y, %H-%M-%S")
            filename = "Presupuesto de Venta {}.pdf".format(str(vvfecha)) #Nombre del archivo

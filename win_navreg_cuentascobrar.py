...
querysql_update = "update cuentascobrar set nroCuenta = {}, montoApertura = {}, RUC_CI = '{}', Direccion = nullif(trim('{}'),''), TelefCel = nullif(trim('{}'),''), ClienteInterno = {} where idclientes = {}"
querysql_insert = "insert into cuentascobrar (nroCuenta, NroFactura, fecha, nroCuota, importe) values ({}, {}, '{}', {}, {})"
querysql_delete = "delete from clientes where idclientes = {}"
querysql_select = "select cc.nroCuenta, cc.NroFactura, fc.idclientes, p.razonsocial, p.ruc_ci, cc.fecha, cc.nroCuota, cc.Importe, cc.importe / cc.nrocuota as mensualidad from cuentascobrar cc inner join ventas_encab fc on cc.NroFactura = fc.NroFactura inner join clientes p on fc.idclientes = p.Idclientes order by nroCuenta"
querysql_selectmax = "select ifnull(max(nroCuenta), 0) + 1 from cuentascobrar"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba
etiquetascol = ["Nro de Cuenta", "Nro Factura", "idCliente", "Razon Social", "RUC-CI", "Fecha Emision", "Nro de Cuotas", "Importe Total", "Mensualidad"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_cuentascobrar.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_cuentascobrar.ui")
    ...
    def clicked_pushButton(self):
        ...
        self.listalineedit_pks_ventas = [self.vedit.lineEdit_3, self.vedit.lineEdit_2, self.vedit.lineEdit_6, self.vedit.lineEdit_7]
        self.run_win_navreg_ventas = win_navreg_ventas.iniciar(self.conex, self.listalineedit_pks_ventas)
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            ...
            filename = "Cuentas Cobrar {}.pdf".format(str(vvfecha)) #Nombre del archivo

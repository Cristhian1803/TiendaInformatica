...
querysql_insert = "insert into arqueocaja (idArqueo, idapertura, montoArqueo, fecha, idUsuarios) values ({}, {}, {}, '{}', {})"
querysql_delete = "delete from clientes where idclientes = {}"
querysql_select = "select ac.idArqueo, c.idapertura, montoArqueo, fecha, ac.idUsuarios, us.Usuario from arqueocaja ac inner join aperturacaja c on ac.Idapertura = c.Idapertura inner join usuarios us on ac.IdUsuarios = us.IdUsuarios order by idapertura"
querysql_selectmax = "select ifnull(max(idarqueo), 0) + 1 from arqueocaja"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba
etiquetascol = ["Codigo Arqueo", "Codigo Apertura", "Monto", "Fecha", "idUsuario", "Usuario"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_arqueo.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_arqueo.ui")
    ...
    def clicked_pushButton(self):
        ...
        self.listalineedit_pks_tim = [self.vedit.lineEdit_3]
        ...
        if len(v0) > 0 and len(v1) > 0 and len(v2) > 0 and len(v3) > 0:
            if v1.isnumeric():
                ...
                try:
                    ...
                    self.vnav.tableWidget.setItem(vfila, 2, celda5)
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            ...
            filename = "Arqueo Caja {}.pdf".format(str(vvfecha)) #Nombre del archivo

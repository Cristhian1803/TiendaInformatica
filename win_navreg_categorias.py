...
querysql_update = "update categorias set idCategoriasProd = {}, DescripcionCatProd = '{}', PorcentajeIVA = {} where idCategoriasProd = {}"
querysql_insert = "insert into categorias (idCategoriasProd, DescripcionCatProd, PorcentajeIVA) values ({}, '{}', {})"
querysql_delete = "delete from categorias where idCategoriasProd = {}"
querysql_select = "select idCategoriasProd, DescripcionCatProd, POrcentajeIVA from categorias order by idCategoriasProd"
querysql_selectmax = "select ifnull(max(idCategoriasProd), 0) + 1 from categorias"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba
etiquetascol = ["Código", "Categoría", "IVA"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        querysql_permiso = "select valor from usuariopermisos where idusuarios like "+idusuario+" and idpermisos like 3"
        cursor.execute(querysql_permiso)
        self.permiso = cursor.fetchone()

        self.vnav = uic.loadUi("win_navreg_categorias.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_categorias.ui")
        ...
        if len(v0) > 0 and len(v1) > 0:
            ...
            try:
                ...
                if self.editando: #Modificando
                    ...
                    cursor.execute(querysql_insert.format(v0, v1, v2)) #SQL ejecutandose NUEVO - 
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            ...
            filename = "categorias {}.pdf".format(str(vvfecha)) #Nombre del archivo
    ...
    def cargardatos(self):
        ...
        for campoid, campodescrip, campo3 in cursor: #

...
querysql_update = "update empleados set idempleados = {}, nombre = '{}', apellido = '{}', cedula = '{}', idcargo = {} where idempleados = {}"
querysql_insert = "insert into empleados (idempleados, nombre, apellido, cedula, idcargo) values ({}, '{}', '{}','{}', {})"
querysql_delete = "delete from empleados where idempleados = {}"
querysql_select = "select e.idempleados, e.nombre, e.apellido, e.cedula, e.idcargo, c.descrip from empleados e inner join cargo c on e.idcargo = c.idcargo order by idempleados"
querysql_selectmax = "select ifnull(max(idempleados), 0) + 1 from empleados"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba
etiquetascol = ["Código", "Nombre", "Apellido", "Cedula", "idCargo", "Cargo"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        querysql_permiso = "select valor from usuariopermisos where idusuarios like "+idusuario+" and idpermisos like 11"
        cursor.execute(querysql_permiso)
        self.permiso = cursor.fetchone()

        self.vnav = uic.loadUi("win_navreg_empleados.ui")
    ...
    def activated_tableWidget(self): #Se activa al darle Enter con una fila seleccionada
        if self.mylistaapegar is None:
            ...
            try:
                ...
                v1 = self.vnav.tableWidget.selectedIndexes()[1].data() + " " + self.vnav.tableWidget.selectedIndexes()[2].data()
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_empleados.ui")
                ...
                self.vedit.lineEdit_6.setMaxLength(200)
                self.vedit.lineEdit_6.setReadOnly(True)
                self.vedit.lineEdit_6_2.setReadOnly(True)
            else:
            ...
            if self.editando:
                ...
                v6 = self.vnav.tableWidget.selectedIndexes()[5].data()#
            ...
            self.vedit.lineEdit_6_2.setText(v6)#
            ...
            self.vedit.btnCiudad.clicked.connect(self.clicked_pushButton_ciudad)
        except:
            self.vnav.statusbar.showMessage("Seleccione un registro para Editar")

    def clicked_pushButton_ciudad(self):
        import win_navreg_cargos
        self.listalineedit_pks_cargo = [self.vedit.lineEdit_6, self.vedit.lineEdit_6_2]
        self.run_win_navreg_cargos = win_navreg_cargos.iniciar(self.conex, self.listalineedit_pks_cargo)
    ...
    def click_actionaceptar(self):
        ...
        v6 = self.vedit.lineEdit_6_2.text()
        vfila = self.filaamodificar
        if len(v0) > 0 and len(v1) > 0 and len(v2) > 0 and len(v3) > 0 and len(v5) > 0:
            ...
            celda6 = QtWidgets.QTableWidgetItem(v6) #
            try:
                cursor = self.conex.cursor() 
                if self.editando: #Modificando
                    cursor.execute(querysql_update.format(v0, v1, v2, v3, v5, self.condicion_update)) #SQL ejecutandose EDITAR
                else: #Creando
                    cursor.execute(querysql_insert.format(v0, v1, v2, v3, v5)) #SQL ejecutandose NUEVO 
                ...
                self.vnav.tableWidget.setItem(vfila, 5, celda6) #
    ...
    def click_actionEliminar(self):
        try: #Pregunta
            txt_valor0 = self.vnav.tableWidget.selectedIndexes()[1].data()
            txt_valor1 = self.vnav.tableWidget.selectedIndexes()[3].data()
            txt_filaAEliminar = self.vnav.tableWidget.currentRow()
            txtmensaje = "<span style='font-size: 14pt; color: #cd1014;'><b>Estas seguro que desea eliminar?</b></span><span style='font-size: 12pt; color: #333333;'><p>Nombre: <b>{}</b><br>Cedula: <b>{}</b></span>".format(txt_valor0, txt_valor1)
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            ...
            filename = "empleados {}.pdf".format(str(vvfecha)) #Nombre del archivo

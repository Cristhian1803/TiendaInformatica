...
v_titulo = "Nota de Débito de Compras"
...
querysql_delete = "delete from notadebitocompra where idnotadebcompra = {}"
querysql_delete2 = "delete from detnotadebcompra where idnotadebcompra = {}"
...
select
    ...
    nc.Fecha,
    nc.idUsuarios, 
    us.usuario from notadebitocompra nc
...
"""
querysql_selectmax = "select ifnull(max(idNotaDebCompra), 0) + 1 from notadebitocompra"
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_notadebitocompras.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_notadebitocompras.ui")
        ...
        if len(v0) > 0 and len(v3) > 0 and len(v7) > 0 and len(v13) > 0 and len(v23) > 0:
            if v0.isnumeric():
                    ...
                    try:
                        ...
                        self.vnav.tableWidget.setItem(vfila, 2, celda7) #idProveedores
                        ...
                        if self.cantfilas > 0:
                            ...
                            filename = "Nota Debito Compra {}.pdf".format(str(vvfecha)) #Nombre del archivo
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            now = datetime.now()
            vvfecha = now.strftime("%d-%m-%Y, %H-%M-%S")
            filename = "Nota Debito Compras {}.pdf".format(str(vvfecha)) #Nombre del archivo

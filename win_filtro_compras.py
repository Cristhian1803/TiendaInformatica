...
select
    ...
    fv.idUsuarios, 
    us.usuario,
    (SELECT SUM(PCompra * cantidad) FROM compras_det where NroFactura = fv.NroFactura) as total
    from compras_encab fv
        inner join proveedores c on fv.idProveedores = c.idProveedores
        inner join usuarios us on fv.IdUsuarios = us.IdUsuarios
        inner join compras_det vd on fv.NroFactura = vd.NroFactura
	group by fv.NroFactura
    order by fv.NroFactura
"""
querysql_select_2 = """
select
    fv.NroFactura,
    ...
    from compras_encab fv
        ...
        inner join usuarios us on fv.IdUsuarios = us.IdUsuarios
        inner join compras_det vd on fv.NroFactura = vd.NroFactura
	where fv.FechaFactura between '{}' AND '{}'
	group by fv.NroFactura
    order by fv.NroFactura
"""
...
etiquetascol = ["Nro Factura", "ID Factura", "idProveedor", "Razón Social", "RUC/CI", "Fecha de Factura", "idUsuario", "Usuario", "Total de Compra"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_filtro_compras.ui")

        self.vnav.label_2.setText("Egresos Totales: ")
    ...
    def clicked_pushButton(self): #Busca por fecha las ventas
        ...
        for campoid, campo4, campo5, \
            ...
            self.vnav.tableWidget.setItem(fila, 1, cel3)
            self.vnav.tableWidget.setItem(fila, 2, cel4) 
            self.vnav.tableWidget.setItem(fila, 3, cel7)
            self.vnav.tableWidget.setItem(fila, 4, cel8)
            self.vnav.tableWidget.setItem(fila, 5, cel9)
            self.vnav.tableWidget.setItem(fila, 6, cel10)
            self.vnav.tableWidget.setItem(fila, 7, cel11)
            self.vnav.tableWidget.setItem(fila, 8, cel12)
            #
            fila += 1
        ...
        for row in range (nb_row):
            colum5 = self.vnav.tableWidget.item(row, 8).text()
    ...
    def click_actionImprimir(self):
        ...
        if self.cantfilas > 0:
            ...
            filename = "Informe Compras {}.pdf".format(str(vvfecha)) #Nombre del archivo

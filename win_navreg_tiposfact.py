...
querysql_update = "update tipos_fact set idTiposFacturas = {}, DescripTiposFact = '{}' where idTiposFacturas = {}"
querysql_insert = "insert into tipos_fact(idTiposFacturas, DescripTiposFact) values ({}, '{}')"
querysql_delete = "delete from tipos_fact where idTiposFacturas = {}"
querysql_select = "select idTiposFacturas, DescripTiposFact from tipos_fact order by idTiposFacturas"
querysql_selectmax = "select ifnull(max(idTiposFacturas), 0) + 1 from tipos_fact"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba
etiquetascol = ["Código", "Nombre"]

class iniciarregtiposfact:
    def __init__(self, conexion, listaapegar = None):
        self.mylistaapegar = listaapegar
        self.conex = conexion
        cursor = self.conex.cursor() #Valida el permiso
        querysql_permiso = "select valor from usuariopermisos where idusuarios like "+idusuario+" and idpermisos like 5"
        cursor.execute(querysql_permiso)
        self.permiso = cursor.fetchone()

        self.vnav = uic.loadUi("win_navreg_tiposfact.ui")
        ...
        #self.vnav.actionPrueba.triggered.connect(self.click_actionPrueba)
    ...
    def cargarventanaedicion(self):
        ...
        try:
            if self.permiso == (1,):
                ...
                self.vedit = uic.loadUi("win_edicion_tiposfact.ui")
        ...
        if len(v0) > 0 and len(v1) > 0:
            ...
            try:
                ...
                if self.editando: #Modificando
                    cursor.execute(querysql_update.format(v0, v1, self.condicion_update)) #SQL ejecutandose
                else: #Creando
                    cursor.execute(querysql_insert.format(v0, v1)) #SQL ejecutandose
    ...
    def click_actionImprimir(self):
        if self.cantfilas > 0:
            ...
            filename = "tiposfactura {}.pdf".format(str(vvfecha)) #Nombre del archivo
 ...
 #   def click_actionPrueba(self):
  #          nb_row = self.vnav.tableWidget.rowCount()
   #         for row in range (nb_row):
    #            colum1 = self.vnav.tableWidget.item(row, 0).text()
     #           colum2 = self.vnav.tableWidget.item(row, 1).text()
      #          print (str(colum1 + " - " + colum2))
    ...
    def cargardatos(self):
        ...
        for campoid, campodescrip in cursor:

...
v_titulo = "Nota de Crédito de Venta"
...
select
    ...
    nc.Fecha,
    nc.idUsuarios, 
    us.usuario from notacreditoventa nc
        inner join ventas_encab fv on nc.NroFactura = fv.NroFactura
        inner join clientes c on fv.idClientes = c.idClientes
        inner join usuarios us on fv.IdUsuarios = us.IdUsuarios
    order by nc.idNotaCredVenta
"""
...
select
    ...
    nc.PVenta,
    nc.Cantidad,
    nc.PVenta * nc.Cantidad as subtotal,
	nc.PorcIva 
    from ventas_det nc
...
"""
querysql_selectmax = "select ifnull(max(idNotaCredVenta), 0) + 1 from notacreditoventa"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba



etiquetascol = ["Codigo", "Nro de Factura", "idCliente", "Razon Social", "RUC-CI", "Fecha", "idUsuario", "Usuario"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_notacreditoventas.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_notacreditoventas.ui")
                self.vedit.label.setText(etiquetascol[0] + ": ")
                self.vedit.label_4.setText("Factura de Venta #: ")
                self.vedit.label_5.setText(etiquetascol[5] + ": ")
            ...
            self.vedit.pushButton_2.setVisible(False)
    ...
    def clicked_pushButton(self):
        ...
        self.listalineedit_pks_venta = [self.vedit.lineEdit_3, self.vedit.lineEdit_4, self.vedit.lineEdit_5, self.vedit.lineEdit_6]
        self.run_win_navreg_ventas = win_navreg_ventas.iniciar(self.conex, self.listalineedit_pks_venta)
        ...
        if len(v0) > 0 and len(v3) > 0 and len(v7) > 0 and len(v13) > 0 and len(v23) > 0:
            if v0.isnumeric():
                    ...
                    try:
                        ...
                        self.vnav.tableWidget.setItem(vfila, 2, celda7) #idCliente
                        self.vnav.tableWidget.setItem(vfila, 3, celda9)
                        self.vnav.tableWidget.setItem(vfila, 4, celda10)
                        self.vnav.tableWidget.setItem(vfila, 5, celda4) #Fecha
                        self.vnav.tableWidget.setItem(vfila, 6, celda13) #idUsuario
                        self.vnav.tableWidget.setItem(vfila, 7, celda14) 
                        ...
                        if self.cantfilas > 0:
                            ...
                            filename = "Nota Credito Venta {}.pdf".format(str(vvfecha)) #Nombre del archivo
    ...
    def cargardatos(self):
        ...
        for campoid, campodescrip, campo3, campo4, campo5, \
            campo8, campo9, campo10 in cursor: #
            cel0 = QtWidgets.QTableWidgetItem(str(campoid))
            ...
            cel7 = QtWidgets.QTableWidgetItem(str(campo8))
            cel8 = QtWidgets.QTableWidgetItem(str(campo9))

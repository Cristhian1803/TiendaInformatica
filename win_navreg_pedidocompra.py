...
querysql_delete = "delete from pedidocompra where idpedidocompra = {}"
querysql_delete2 = "delete from detpedidocompra where idpedidocompra = {}"
...
select
    ...
    p.razonsocial,
    p.ruc_ci,
    pr.fecha from Pedidocompra pr
        inner join proveedores p on pr.idProveedores = p.idProveedores
    order by pr.idPedidoCompra
"""
querysql_selectmax = "select ifnull(max(idpedidocompra), 0) + 1 from pedidocompra"
#querysql_selectmax = None --- Habilitar si no se necesita el autoincrementable de arriba



etiquetascol = ["ID Pedido de Compra", "id Proveedor", "Razón Social", "RUC-CI", "Fecha"]
...
class iniciar:
    def __init__(self, conexion, listaapegar = None):
        ...
        self.vnav = uic.loadUi("win_navreg_pedidocompra.ui")
    ...
    def cargarventanaedicion(self):
        ...
        try:
            ...
            if self.permiso == (1,):
                self.vedit = uic.loadUi("win_edicion_pedidocompra.ui")
